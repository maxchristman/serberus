...
#ifdef OURS
				if (SecT.has_secret())
				{ // has secret check should be unnecessary if default is no secrets
	...
	const auto cull_sts = [] (std::vector<Alg::ST>& stvec) {
	  ...
	  llvm::copy(sts, std::back_inserter(stvec));
				...
				CountStat stat_nonspec_secrets(log, "maybe_nonspeculative_secrets",
											   llvm::count_if(llvm::instructions(F), [&](auto &I)
															  { return NST.secret(&I); }));
				CountStat stat_nonspec_publics(log, "definitely_nonspeculative_public",
											   llvm::count_if(util::nonvoid_instructions(F), [&](auto &I)
															  { return !NST.secret(&I); }));
